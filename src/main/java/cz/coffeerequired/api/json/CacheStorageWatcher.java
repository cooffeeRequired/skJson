...
import cz.coffeerequired.api.json.bukkit.EventFileWatcher;
...
public class CacheStorageWatcher {
            ...
            while ((key = watchService.take()) != null) {
                for (WatchEvent<?> event : key.pollEvents()) {
                    ...
                    if (event.context().equals(file.toPath().getFileName()) && !Objects.equals(potentialJsonContent, jsonifyFile)) {
                        ...
                        this.event.setContent(jsonifyFile);
                        cache.replace(id, map);

                        //? TODO Some debug {Modified}?
                    } else {
                        // ? TODO some debug is cached!?
                    }
                    break;
                }
    ...
    private void invokeEvent(EventFileWatcher event) {
        assert event != null;
        this.event = event;
    }
