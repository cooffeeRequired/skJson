...
public class CondJsonHas extends Condition {
    ...
    public boolean check(@NotNull Event e) {
        ...
        if (json instanceof JsonObject) {
            if (mark == 1) {
                return (pattern == 0) == JsonUtils.check(json.getAsJsonObject(), search, type == 1 ? JsonUtils.Type.KEY : JsonUtils.Type.VALUE);
            } else {
                for (Object search0 : searches) {
                    match = (pattern == 0) == JsonUtils.check(json.getAsJsonObject(), search0.toString(), type == 1 ? JsonUtils.Type.KEY : JsonUtils.Type.VALUE);
            ...
            if (mark == 1) {
                return (pattern == 0) == JsonUtils.check(json.getAsJsonArray(), search, type == 1 ? JsonUtils.Type.KEY : JsonUtils.Type.VALUE);
            } else {
                for (Object search0 : searches) {
                    match = (pattern == 0) == JsonUtils.check(json.getAsJsonArray(), search0.toString(), type == 1 ? JsonUtils.Type.KEY : JsonUtils.Type.VALUE);
                    if (!match) return false;
                }
                return match;
            }
