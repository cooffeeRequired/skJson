...
public class EffSave extends Effect {
    ...
    protected void execute(@NotNull Event e) {
        ...
        if (pattern == 1) {
            JSON_STORAGE.forEach((key, value) -> {
                File fileLink = FILE_JSON_MAP.get(key);
            ...
            if (JSON_STORAGE.containsKey(externalID) && FILE_JSON_MAP.containsKey(externalID)) {
                JsonElement value = JSON_STORAGE.get(externalID);
                File fileLink = FILE_JSON_MAP.get(externalID);
                jfh.writeFile(fileLink, value, async);
            }
        }
    }
