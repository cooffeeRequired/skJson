...
import static cz.coffee.adapter.Adapter.SERIALIZED_JSON_TYPE_KEY;
...
public class DefaultAdapter {
     ...
     * </p>
     */
    private static final Adapter<Chunk> CHUNK_ADAPTER = new Adapter<>() {
        @Override
        public @NotNull JsonElement toJson(Chunk source) {
            final JsonObject parsed = new JsonObject();
            parsed.addProperty(SERIALIZED_JSON_TYPE_KEY, source.getClass().getName());
            parsed.addProperty("world", source.getWorld().getName());
            parsed.addProperty("x", source.getX());
            parsed.addProperty("z", source.getZ());
            return !parsed.isEmpty() ? parsed : JsonNull.INSTANCE;
        }
     ...
     * </p>
     */
    private static final Adapter<World> WORLD_ADAPTER = new Adapter<>() {
        @Override
        public @NotNull JsonElement toJson(World source) {
            JsonObject parsed = new JsonObject();
            parsed.addProperty(SERIALIZED_JSON_TYPE_KEY, source.getClass().getName());
            parsed.addProperty("name", source.getName());
            return !parsed.isEmpty() ? parsed : JsonNull.INSTANCE;
        }
    ...
    private static final Adapter<NBTContainer> NBT_ADAPTER = new Adapter<>() {
        @Override
    ...
    private static final Adapter<ItemStack> ITEM_ADAPTER = new Adapter<>() {
    ...
    private static final Adapter<ItemMeta> META_ADAPTER = new Adapter<>() {
     ...
     * </p>
     */
    private static final Adapter<Inventory> INVENTORY_ADAPTER = new Adapter<>() {
    ...
    public static <T> JsonElement assignTo(T object) {
        ...
        else if (object instanceof NBTCompound) return NBT_ADAPTER.toJson(new NBTContainer(object.toString()));
        else return null;
    }
    ...
    public static JsonElement parse(Object item, Event e) {
        ...
        return DefaultAdapter.assignTo(finalI);
    }
