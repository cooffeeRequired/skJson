        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (!file.getParentFile().exists() && !file.getParentFile().mkdirs()) {
                    LoggingUtil.error(fstring("Cannot create a directory %s", file.getParentFile().getAbsoluteFile()));
                }
                if (file.exists()) {
                    LoggingUtil.warn(fstring("Cannot create a file %s cause the file already exists.", filePath));
                    return false;
                }
