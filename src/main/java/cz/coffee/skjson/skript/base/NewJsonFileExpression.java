...
public class NewJsonFileExpression extends SimpleExpression<JsonElement> {

    static {
        Skript.registerExpression(NewJsonFileExpression.class, JsonElement.class, ExpressionType.COMBINED,
                "json from [(0:(text|string)|1:([json]|:yaml) file)|2:web[site] [file]] %objects%");
    }
                ...
                if (value instanceof JsonElement json) {
                    ...
                    try {
                        if ((value instanceof ItemType type) && type.getTypes().size() > 1) {
                            type.getTypes().forEach(data -> {
                                output.add(ParserUtil.parse(data));
                            });
                        } else {
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return "json from " + switch (mark) {
            case 0 -> "text";
            case 1 -> isYaml ? "yaml file" : "json file";
            case 2 -> "website file";
            default -> null;
