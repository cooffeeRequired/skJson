...
public abstract class Logger {
    ...
    public static boolean versionError(Version userVersion, Version neededVersion, boolean disablePlugin, PluginManager manager, JavaPlugin plugin) {
        if (userVersion.isSmallerThan(new Version(2, 8, 0))) {
            Bukkit.getConsoleSender().sendMessage(cstring("Running on older version than 2.8, please consider update"));
        }

        if (userVersion.isSmallerThan(neededVersion)) {
            Bukkit.getConsoleSender().sendMessage(cstring(PLUGIN_PREFIX + ERROR_PREFIX + "&c-----------------------------------------------------------------------------------------------------------"));
            Bukkit.getConsoleSender().sendMessage(cstring(PLUGIN_PREFIX + ERROR_PREFIX + "&cThis version doesn't support a older version of srkipt " + userVersion));
            Bukkit.getConsoleSender().sendMessage(cstring(PLUGIN_PREFIX + ERROR_PREFIX + "&eUse older version &fhttps://github.com/SkJsonTeam/skJson/releases/tag/2.8.6"));
            Bukkit.getConsoleSender().sendMessage(cstring(PLUGIN_PREFIX + ERROR_PREFIX + "Or update skript to &f2.7+"));
            Bukkit.getConsoleSender().sendMessage(cstring(PLUGIN_PREFIX + ERROR_PREFIX + "&c-----------------------------------------------------------------------------------------------------------"));
